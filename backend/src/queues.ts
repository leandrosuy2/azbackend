...
async function handleVerifySchedules(job) {
  try {
    /** Antes: sendAt entre "agora" e "agora+30s" — a maioria dos horários nunca batia; atrasados também não. */
    ...
    if (count > 0) {
      for (const schedule of schedules) {
        try {
          ...
          logger.info(`Disparo agendado para: ${schedule.contact?.name || schedule.id}`);
        } catch (rowErr: any) {
          Sentry.captureException(rowErr);
          logger.error("ScheduleMonitor -> fila por agendamento", rowErr.message);
        }
      }
    }
  } catch (e: any) {
...
async function handleSendScheduledMessage(job) {
  const scheduleIdFromJob = job?.data?.schedule?.id;
  if (!scheduleIdFromJob) {
    logger.error("SendScheduledMessage: job sem schedule.id");
    return;
  }
  ...
  try {
    scheduleRecord = await Schedule.findByPk(scheduleIdFromJob, {
      include: [
    ...
    Sentry.captureException(e);
    logger.info(`Erro ao tentar consultar agendamento: ${scheduleIdFromJob}`);
  }

  if (!scheduleRecord || !scheduleRecord.contact) {
    logger.error(
      `Agendamento ${scheduleIdFromJob} inexistente ou sem contato (dados da fila Bull não trazem relações).`
    );
    return;
  }
  ...
  try {
    ...
    if (!whatsapp) {
      whatsapp = await GetDefaultWhatsApp(undefined, schedule.companyId);
    }
...
function handleKanbanPrazoLembretes() {
  const job = new CronJob(
    "0 */30 * * * *",
    async () => {
      try {
        await ProcessKanbanPrazoLembretesJob();
      } catch (e) {
        logger.error("handleKanbanPrazoLembretes", e);
      }
    },
    null,
    true,
...
handleKanbanPrazoLembretes();

function handleKanbanAgendadoLembretes() {
  const job = new CronJob(
    "0 * * * * *",
    async () => {
      try {
        await ProcessKanbanAgendadoLembretesJob();
      } catch (e) {
        logger.error("handleKanbanAgendadoLembretes", e);
      }
    },
    null,
    true,
