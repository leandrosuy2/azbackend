...
const getContactMessage = async (msg: WAMessage, wbot: Session) => {
  ...
  const peerDisplay =
    msg.pushName || (msg as any).verifiedBizName || "";
  return isGroup
    ? {
        id: getSenderMessage(msg, wbot),
        name: msg.pushName || (msg as any).verifiedBizName,
      }
    : {
        id: msg.key.remoteJid,
        name: msg.key.fromMe ? rawNumber : peerDisplay,
      };
};
...
const wbotMessageListener = (wbot: Session, companyId: number): void => {
  ...
  wbot.ev.on("contacts.update", (contacts: any) => {
    contacts.forEach(async (contact: any) => {
      ...
      // Baileys emite updates só com notify (push name) sem imgUrl — antes ignorávamos e o nome ficava vazio.
      const hasImg = typeof contact.imgUrl !== "undefined";
      const hasNotify = typeof contact.notify !== "undefined";
      const hasName = typeof contact.name !== "undefined";
      const hasVerified = typeof contact.verifiedName !== "undefined";
      const hasPhoneNumber = typeof contact.phoneNumber !== "undefined";
      ...
      const waLabel =
        ...
        (hasVerified && contact.verifiedName) ||
        "";

      let remoteJidForStorage = String(contact.id);
      let numberDigits =
        remoteJidForStorage.split("@")[0]?.replace(/\D/g, "") || "";

      if (
        hasPhoneNumber &&
        contact.phoneNumber &&
        String(contact.phoneNumber).includes("@s.whatsapp.net")
      ) {
        try {
          const pn = jidNormalizedUser(String(contact.phoneNumber));
          numberDigits = pn.split("@")[0].replace(/\D/g, "");
          remoteJidForStorage = pn;
        } catch {
          /* mantém id */
        }
      }

      const idIsLid = isLidUser(String(contact.id));
      const initialName =
        typeof waLabel === "string" && waLabel.trim().length > 0
          ? waLabel.trim()
          : idIsLid
            ? ""
            : numberDigits;
