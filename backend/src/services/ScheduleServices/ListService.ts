...
interface Request {
  ...
  /** Filtro opcional: withTicket = reabre ticket; directOnly = envio sem reabrir */
  scheduleKind?: string;
}
...
const ListService = async ({
  ...
  if (scheduleKind === "withTicket") {
    whereCondition = {
      ...whereCondition,
      openTicket: "enabled"
    };
  } else if (scheduleKind === "directOnly") {
    whereCondition = {
      ...whereCondition,
      openTicket: { [Op.ne]: "enabled" }
    };
  }
