...
const planIncludeForCompanyJson = {
  model: Plan,
  as: "plan" as const,
  required: false,
  attributes: [
...
const reloadCompanyWithPlan = async (company: Company): Promise<Company> => {
  await company.reload({ include: [planIncludeForCompanyJson] });
  return company;
};
...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  await reloadCompanyWithPlan(company);
...
export const listPlan = async (req: Request, res: Response): Promise<Response> => {
  ...
  const jsonCompany = (company: Company) => {
    const plain = company.get({ plain: true }) as Record<string, unknown>;
    return {
      ...plain,
  ...
  if (requestUser.super === true) {
    ...
    return res.status(200).json(jsonCompany(company));
  }
...
export const uploadLogo = async (
  ...
  if (!requestUser.super && String(companyId) !== String(id)) {
  ...
  if (previous && previous !== file.filename) {
    try {
      ...
      /* ignorar falha ao apagar logo antigo */
    }
  }
...
export const indexPlan = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (requestUser.super === true) {
    const { companies, count, hasMore } = await ListCompaniesPlanService({
      searchParam,
      pageNumber
    });
    return res.json({ companies, count, hasMore });
