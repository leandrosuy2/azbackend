  ...
  useEffect(() => {
    ...
    const getSetting = async () => {
      try {
        ...
        setUserToken(response ? "disabled" : "disabled");
      } catch (_) {
    ...
    getSetting();
    // settings.get é estável o suficiente; não incluir o objeto settings (novo a cada render)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user?.id]);
              ...
              <>
                ...
                <b>
                  {(user?.company?.name || "").trim() ||
                    i18n.t("mainDrawer.appBar.user.companyNameFallback")}
                </b>
