...
const NotificationsPopOver = (volume) => {
	...
	useEffect(() => {
		...
		if (!user.id || !isSocketClientReady(socket)) {
			return;
		}
		const queueIds = queues.map((q) => q.id);
			...
			const onKanbanLembrete = (data) => {
				const dt = (data.destinoTipo || "interno").toLowerCase();
				if (dt === "responsavel" || dt === "usuario") {
					if (data.targetUserId == null || Number(data.targetUserId) !== Number(user.id)) {
						return;
					}
				} else if (dt === "fila" || dt === "grupo" || dt === "queue") {
					if (data.destinoId == null || !queueIds.includes(Number(data.destinoId))) {
						return;
					}
				} else if (dt === "contato_whatsapp" || dt === "whatsapp" || dt === "contato") {
				...
				toast.info(
					<span>
						...
						{waHint ? (
							<>
								<br />
								<small style={{ opacity: 0.85 }}>{waHint}</small>
							</>
						) : null}
					</span>,
					{
