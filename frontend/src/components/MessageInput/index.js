...
const MessageInput = ({
  ...
  const privateMessageRef = useRef(privateMessage);
  const signMessageRef = useRef(signMessage);
  const replyingMessageRef = useRef(replyingMessage);
  const editingMessageRef = useRef(editingMessage);
  const handleUploadQuickMessageMediaRef = useRef(null);
  const deliverQuickMessageRef = useRef(async () => {});
  ...
  const handleQuickAnswersClick = async (value) => {
    const composeOnly = value.autoSend === false;
    await deliverQuickMessageRef.current(value, {
      composeOnly,
      closeTypeBar: true,
    });
  };
    ...
    async function fetchData() {
      ...
      const options = visible.map((m) => {
        ...
        return {
          id: m.id,
          value: m.message,
          label: `/${m.shortcode}${catBit} - ${truncatedMessage}`,
    ...
    if (
      ...
      if (firstWord === "/") {
        ...
        const filteredOptions = quickAnswers.filter((m) => {
          ...
          if (needle && cat && cat.indexOf(needle) > -1) return true;
          const sc = String(m.shortcode || "").toLowerCase();
          if (needle && sc && sc.indexOf(needle) > -1) return true;
          return false;
        });
  ...
  const handleUploadQuickMessageMedia = async (
    ...
    try {
      ...
      if (isMounted.current) {
        ...
        if (quickMessageId) {
          try {
            await api.post(`/quick-messages/${quickMessageId}/increment-use`);
          } catch (_) {
            /* ignore */
          }
        }
      ...
      if (manageLoading && isMounted.current) {
        setLoading(false);
      }
    }
  };

  handleUploadQuickMessageMediaRef.current = handleUploadQuickMessageMedia;

  deliverQuickMessageRef.current = async (quickMsg, { composeOnly, closeTypeBar = false }) => {
    const u = userRef.current;
    const pm = privateMessageRef.current;
    const sm = signMessageRef.current;
    const qm = replyingMessageRef.current;
    const em = editingMessageRef.current;
    const tid = ticketIdRef.current;

    const seq = parseQuickMessageSequence(quickMsg.message || "");

    if (seq) {
      const onlyCompose = composeOnly || quickMsg.autoSend === false;
      const hasMediaStep = seq.steps.some((s) => s.type === "media");
      const hasSeqFiles =
        Boolean(quickMsg.mediaPath) || parseAttachmentsList(quickMsg.attachments).length > 0;

      if (onlyCompose) {
        if (hasMediaStep && hasSeqFiles) {
          toast.info(i18n.t("messagesInput.quickReplies.sequenceComposeBlocked"));
        } else if (hasMediaStep) {
          toast.info(i18n.t("messagesInput.quickReplies.sequenceComposeHint"));
        }
        const firstText = seq.steps.find(
          (s) => s.type === "text" && String(s.body || "").trim()
        );
        const ft = firstText
          ? formatQuickMessageTemplate(String(firstText.body || ""), {
      ...
      if (!tid) {
        toast.warning("Aguarde o carregamento do ticket para enviar a resposta.");
        return;
      }

      setLoading(true);
      try {
        let pendingQuoted = qm;
        for (const step of seq.steps) {
          if (step.type === "delay") {
            await new Promise((r) => setTimeout(r, Math.max(0, Number(step.seconds) || 0) * 1000));
            continue;
          }
          if (step.type === "text") {
            const formatted = formatQuickMessageTemplate(String(step.body || ""), {
              ticket: ticketRef.current,
              contact: contactRef.current,
              user: u,
            }).trim();
            if (!formatted) continue;
            const userName = pm ? `${u.name} - Mensagem Privada` : u.name;
            const body =
              (sm || pm) && !em ? `*${userName}:*\n${formatted}` : formatted;
            const message = {
              read: 1,
              ...
              mediaUrl: "",
              body,
              quotedMsg: pendingQuoted,
              isPrivate: pm ? "true" : "false",
            };
            pendingQuoted = null;
            await api.post(`/messages/${tid}`, message);
          }
          if (step.type === "media") {
            const idx = typeof step.mediaIndex === "number" ? step.mediaIndex : 0;
            const list = parseAttachmentsList(quickMsg.attachments);
            const row = list[idx];
            const mediaUrl = row?.path
              ? buildQuickMessageAttachmentPublicUrl(quickMsg, row.path)
              : idx === 0
                ? quickMsg.mediaPath
                : null;
            if (!mediaUrl) {
              toast.info(i18n.t("messagesInput.quickReplies.sequenceMediaMissing"));
              continue;
            }
            try {
              ...
              const cap = formatQuickMessageTemplate(String(step.caption || ""), {
                ticket: ticketRef.current,
                contact: contactRef.current,
                user: u,
              });
              const upload = handleUploadQuickMessageMediaRef.current;
              if (typeof upload === "function") {
                await upload(data, cap, null, { manageLoading: false });
              }
            } catch (err) {
              toastError(err);
            }
        ...
        if (quickMsg.id) {
          try {
            await api.post(`/quick-messages/${quickMsg.id}/increment-use`);
          } catch (_) {
            /* ignore */
          }
        }
        if (closeTypeBar) setTypeBar(false);
      } catch (err) {
        toastError(err);
      } finally {
    ...
    const formatted = formatQuickMessageTemplate(quickMsg.message || "", {
      ticket: ticketRef.current,
      contact: contactRef.current,
      user: u,
    });
    ...
    if (onlyCompose) {
      if (quickMsg.mediaPath) {
        toast.info(
          "Resposta com mídia e envio automático desligado: use o texto no campo ou ative o envio automático na edição da resposta rápida."
        );
      }
    ...
    if (quickMsg.mediaPath) {
      try {
        const { data } = await axios.get(quickMsg.mediaPath, {
          responseType: "blob",
        });
        const upload = handleUploadQuickMessageMediaRef.current;
        if (typeof upload === "function") {
          await upload(data, formatted, quickMsg.id);
          sentMedia = true;
        }
      } catch (err) {
        toastError(err);
        ...
        toast.info("Enviando só o texto (anexo não pôde ser carregado).");
      }
    }
  ...
  privateMessageRef.current = privateMessage;
  signMessageRef.current = signMessage;
  replyingMessageRef.current = replyingMessage;
  editingMessageRef.current = editingMessage;

  useEffect(() => {
    if (!quickReplyHandlersRef) return undefined;

    quickReplyHandlersRef.current.sendQuickReply = async (quickMsg, { composeOnly } = {}) => {
      await deliverQuickMessageRef.current(quickMsg, { composeOnly, closeTypeBar: false });
    };

    return () => {
      if (quickReplyHandlersRef.current) {
        delete quickReplyHandlersRef.current.sendQuickReply;
      }
    };
  }, [quickReplyHandlersRef]);
            ...
            <Hidden only={["sm", "xs"]}>
              ...
              >
                ...
                >
                  <Timer
                    fontSize="small"
                    style={{ marginRight: 12, color: "grey" }}
                  />
                  ...
                  <Tooltip
                    title={
                      quickRepliesOpen
                        ...
                        : i18n.t("messagesInput.quickReplies.toolbarButton")
                    }
                  >
                    <IconButton
                      aria-label="quickRepliesOpen"
                      component="span"
                      disabled={disableOption()}
                      onClick={onToggleQuickReplies}
                      style={{
                        color:
                          quickRepliesOpen && theme.mode === "light"
                    ...
                    >
                      <FlashOn className={classes.sendMessageIcons} />
                    </IconButton>
                  </Tooltip>
                ) : (
